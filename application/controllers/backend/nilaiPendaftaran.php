<?php
class nilaiPendaftaran extends CI_Controller{
    
    var $folder =   "backend/nilaiPendaftaran";
	...
	{	
		...
		foreach ($list as $regCalonSiswa) {
			...
			$row[] = $regCalonSiswa->reg_data_nilai_tes_baca_tulis_alquran;
			$row[] = $regCalonSiswa->reg_data_nilai_tes_ahlaq;
			$row[] = $regCalonSiswa->reg_data_nilai_tes_minat_bakat;
			$row[] = $regCalonSiswa->reg_data_nilai_tes_bhs_inggris;
	...
	function saveNilai()
	{
		...
		$sqlTblRegDataNilaiTes =  $this->db->get_where('tbl_reg_data_nilai_tes',array('reg_data_nilai_tes_reg_akun_id'=>$id));
		if($sqlTblRegDataNilaiTes->num_rows()>0) {
			$dataNilai = $this->db->query("SELECT * FROM tbl_reg_data_nilai_tes WHERE reg_data_nilai_tes_reg_akun_id = ". $id ."")->result();
			$nilaiRaport;
			$nilaiPrestasi;
			foreach ($dataNilai as $n)
			{
				$nilaiRaport = $n->reg_data_nilai_raport;
				$nilaiPrestasi = $n->reg_data_nilai_prestasi;	
			}
			$totalNilai = $nilaiWawancara + $nilaiRaport + $nilaiPrestasi;
			$data = array(
			...
			$data = array(
				...
				'reg_data_nilai_tes_bhs_inggris' => $nilaiInggris,
				'reg_data_nilai_wawancara' => round($nilaiWawancara, 1),
				'reg_data_nilai_total' => round($nilaiWawancara, 1)
			);
		   ...
		   for ($row = 2; $row <= $highestRow; $row++){  
			 $rowData = $sheet->rangeToArray('A' . $row . ':' . $highestColumn . $row,
			   ...
			   FALSE);
				...
				{
					$id = $s->reg_akun_id;
					$nilaiQuran = $rowData[0][1];
					$nilaiAhlaq = $rowData[0][2];
					$nilaiMinat = $rowData[0][3];
					$nilaiInggris = $rowData[0][4];
					$nilaiWawancara = (($nilaiQuran + $nilaiAhlaq + $nilaiInggris + $nilaiMinat) / 130) * 100;
					$totalNilai;
					$sqlTblRegDataNilaiTes =  $this->db->get_where('tbl_reg_data_nilai_tes',array('reg_data_nilai_tes_reg_akun_id'=>$id));
					if($sqlTblRegDataNilaiTes->num_rows()>0) {
