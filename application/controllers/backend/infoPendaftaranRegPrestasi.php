<?php
class infoPendaftaranRegPrestasi extends CI_Controller{
    
    var $folder =   "backend/infoPendaftaranRegPrestasi";
    ...
    function index()
    {
        ...
        $data['desc'] = "";
        $data['info'] = "";
        $data['judulHalaman'] = "Data Calon Siswa (Reguler - Prestasi) - SIAKAD ONLINE";
        $data['menu'] = "infoPendaftaran";
        $data['subMenu'] = "infoPendaftaranRegPrestasi";
        $data['subMenuB'] = "";
	...
	{	
		$list = $this->regCalonSiswaModel->get_datatables('A');
		$data = array();
		$no = 0;
		$jalurDaftar = "";
		$status = "";
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->regCalonSiswaModel->count_all('A'),
						"recordsFiltered" => $this->regCalonSiswaModel->count_filtered('A'),
						"data" => $data,
				);
