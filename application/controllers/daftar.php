...
class daftar extends CI_Controller
{
	...
	function save()
	{
		...
		$jalur = $this->input->post('reg_akun_jalur_daftar');
		$noreg = '';
		$ketReg = '';
		if($hsl <= 0){
			if($jalur == 'P'){
				$noreg = (mysql_num_rows(mysql_query("SELECT * FROM tbl_reg_akun WHERE reg_akun_no_reg NOT LIKE '%regular_%'"))) + 1;
				$ketReg = '/ppdb'.$namaThnAjaranTmp;
			} else if ($jalur == 'R'){
			...
			$data = array(
				...
				'reg_akun_password_tmp' => $this->input->post('reg_akun_password'),
				'reg_akun_jalur_daftar' => $jalur
			);
