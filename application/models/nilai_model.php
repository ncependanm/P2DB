...
class Nilai_Model extends CI_Model {
	...
	var $column_order = array('kelas_siswa_tmp_id','siswa_nisn','siswa_nama',null); //set column field database for datatable orderable
	...
	private function _get_datatables_query($idRuangan, $idThnAjaran, $idMataPelajaran, $idGuru)
	{
		$this->db->from($this->table);
		$sqlOR = "kelas_siswa_tmp_ruangan_id = ". $idRuangan ." 
			AND kelas_siswa_tmp_thn_ajaran_id = ". $idThnAjaran ." 
			AND nilai_mata_pelajaran_id = ". $idMataPelajaran ." 
			AND guru_id = ". $idGuru ."";
		$this->db->where($sqlOR);
		$this->db->join('tbl_nilai', 'tbl_kelas_siswa_tmp.kelas_siswa_tmp_id=tbl_nilai.nilai_kelas_siswa_tmp_id', 'left');
	...
	function get_datatables($idRuangan, $idThnAjaran, $idMataPelajaran, $idGuru)
	{
		$this->_get_datatables_query($idRuangan, $idThnAjaran, $idMataPelajaran, $idGuru);
		if($_POST['length'] != -1)
	...
	function count_filtered($idRuangan, $idThnAjaran, $idMataPelajaran, $idGuru)
	{
		$this->_get_datatables_query($idRuangan, $idThnAjaran, $idMataPelajaran, $idGuru);
		$query = $this->db->get();
