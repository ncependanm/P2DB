...
<script src="<?=base_url();?>asset/pages/js/form-validation-pendidikan.js" type="text/javascript"></script>
...
$(document).ready(function() {
	...
	table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/pendidikan/loadTable')?>",
            "type": "POST"
        },
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/pendidikan/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			showInputan();
            $('[name="id"]').val(data.pendidikan_id);
            $('[name="pendidikan_nama"]').val(data.pendidikan_nama);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/pendidikan/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
        url = "<?php echo site_url('backend/pendidikan/update')?>";
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/pendidikan/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				load();
                                    ...
                                    <form id="form" class="form-horizontal" autocomplete="off">
                                        ...
                                        <div class="form-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-4">
                                                    <input type="text" name="pendidikan_nama" data-required="1" class="form-control" /> 
												</div>
                                            </div>
                                        </div>
