...
<script src="<?=base_url();?>asset/pages/js/form-validation-kota.js" type="text/javascript"></script>
...
$(document).ready(function() {
	...
	table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/kota/loadTable')?>",
            "type": "POST"
        },
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/kota/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			showInputan();
            $('[name="id"]').val(data.kota_id);
            $('[name="kota_propinsi_id"]').val(data.kota_propinsi_id);
            $('[name="kota_nama"]').val(data.kota_nama);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/kota/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
        url = "<?php echo site_url('backend/kota/update')?>";
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/kota/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				load();
                                    ...
                                    <form id="form" class="form-horizontal" autocomplete="off">
                                        ...
                                        <div class="form-body">
                                            ...
                                            <div class="form-group">
                                                <label class="control-label col-md-3">Propinsi
                                                    <span class="required"> * </span>
                                                </label>
                                                <div class="col-md-4">
												    <select class="form-control select2me" name="kota_propinsi_id">
													<!--<select class="form-control" name="kota_propinsi_id">-->
											...
											<div class="form-group">
                                                ...
                                                <div class="col-md-4">
                                                    <input type="text" name="kota_nama" data-required="1" class="form-control" /> 
												</div>
                                            </div>
                                        </div>
