...
<script src="<?=base_url();?>asset/pages/js/form-validation-pekerjaan.js" type="text/javascript"></script>
...
$(document).ready(function() {
	...
	table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/pekerjaan/loadTable')?>",
            "type": "POST"
        },
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/pekerjaan/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="pekerjaan_nama"]').val(data.pekerjaan_nama);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/pekerjaan/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
        url = "<?php echo site_url('backend/pekerjaan/update')?>";
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/pekerjaan/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				load();
                                    ...
                                    <form id="form" class="form-horizontal" autocomplete="off">
                                        ...
                                        <div class="form-body">
                                            ...
                                            <div class="form-group">
                                                <label class="control-label col-md-5">Pekerjaan
                                                    <span class="required"> * </span>
                                                </label>
                                                <div class="col-md-7">
                                                    <input type="text" name="pekerjaan_nama" data-required="1" class="form-control" /> 
												</div>
                                            </div>
                                        </div>
