...
<script src="<?=base_url();?>asset/pages/js/form-validation-sekolah.js" type="text/javascript"></script>
...
$(document).ready(function() {
	...
	table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/sekolah/loadTable')?>",
            "type": "POST"
        },
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/sekolah/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			showInputan();
            $('[name="id"]').val(data.sekolah_id);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/sekolah/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
        url = "<?php echo site_url('backend/sekolah/update')?>";
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/sekolah/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				load();
                                    ...
                                    <form id="form" class="form-horizontal" autocomplete="off">
                                        ...
                                        <div class="form-body">
                                            ...
                                            <div class="form-group">
                                                <label class="control-label col-md-5">Nama Sekolah
                                                    <span class="required"> * </span>
                                                </label>
                                                <div class="col-md-7">
                                                    <input type="text" name="sekolah_nama" data-required="1" class="form-control" /> 
												</div>
                                            </div>
                                        </div>
